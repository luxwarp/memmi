...
EMBEDDING_REQUESTS = Counter("memmi_embedding_requests_total", "Total number of embedding requests", ["provider"])
EMBEDDING_ERRORS = Counter("memmi_embedding_errors_total", "Total number of embedding errors", ["provider"])
EMBEDDING_LATENCY = Histogram("memmi_embedding_latency_seconds", "Latency of embedding generation", ["provider"])

RETRIEVAL_REQUESTS = Counter("memmi_retrieval_requests_total", "Total number of retrieval calls", [])
RETRIEVAL_ERRORS = Counter("memmi_retrieval_errors_total", "Total number of retrieval errors", [])
RETRIEVAL_LATENCY = Histogram("memmi_retrieval_latency_seconds", "Latency of retrieval execution", [])
...
class EmbeddingProvider(ABC):
    @abstractmethod
    async def get_embedding(self, text: str, session: Optional[aiohttp.ClientSession] = None) -> Optional[np.ndarray]: pass
    @abstractmethod
    async def get_embeddings_batch(self, texts: List[str], session: Optional[aiohttp.ClientSession] = None) -> List[Optional[np.ndarray]]: pass
...
class LocalEmbeddingProvider(EmbeddingProvider):
    ...
    async def get_embeddings_batch(self, texts: List[str], session: Optional[aiohttp.ClientSession] = None) -> List[Optional[np.ndarray]]:
        if not self.model or not texts: return [None] * len(texts)
        try:
            loop = asyncio.get_running_loop()
            embeddings = await loop.run_in_executor(None, lambda: self.model.encode(texts, normalize_embeddings=True, show_progress_bar=False))
    ...
    async def get_embedding(self, text: str, session: Optional[aiohttp.ClientSession] = None) -> Optional[np.ndarray]:
        try:
            ...
            try:
                ...
                async with inner_session.post(self.api_url, json=data, headers=headers, timeout=30) as response:
                ...
                if should_close: await inner_session.close()
        except Exception as e:
    ...
    async def get_embeddings_batch(self, texts: List[str], session: Optional[aiohttp.ClientSession] = None) -> List[Optional[np.ndarray]]:
        try:
            ...
            try:
                ...
                async with inner_session.post(self.api_url, json=data, headers=headers, timeout=60) as response:
                    if response.status == 200:
                        res_json = await response.json()
                        if "data" in res_json:
...
class EmbeddingManager:
    ...
    def _get_lock(self, user_id: str) -> asyncio.Lock:
        if user_id not in self._locks: self._locks[user_id] = asyncio.Lock()
    ...
    def _ensure_provider(self):
        ...
        if not self.provider or self._current_provider_type != valves.embedding_provider_type:
            ...
            if valves.embedding_provider_type == "local":
                ...
                self.provider = OpenAICompatibleEmbeddingProvider(valves.embedding_api_url, valves.embedding_api_key, valves.embedding_model_name)

    async def _get_embedding_ollama(self, text: str) -> Optional[np.ndarray]:
        """Native Ollama API Handler for MEMMI"""
        try:
            valves = self.get_valves()
            base_url = valves.embedding_api_url.rstrip("/")
            url = f"{base_url}/api/embeddings"
            payload = {"model": valves.embedding_model_name, "prompt": text}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, headers=headers) as resp:
                    if resp.status != 200:
                        ...
                        print(f"[MEMMI Core] Ollama Error: {resp.status} - {error_text}")
                        return None
                    data = await resp.json()
                    # Return numpy array directly
                    return np.array(data.get("embedding"))
        except Exception as e:
            print(f"[MEMMI Core] Ollama embedding failed: {e}")
            return None
    ...
    async def get_embedding(self, text: str) -> Optional[np.ndarray]:
        ...
        if not self.provider: self._ensure_provider()
    ...
    async def get_embeddings_batch(self, texts: List[str]) -> List[Optional[np.ndarray]]:
        if not self.provider: self._ensure_provider()
        if not self.provider: return [None] * len(texts)
        self._ensure_session()
...
class MemoryPipeline:
    ...
    async def identify_memories(self, user_message: str, context_memories: List[Dict[str, Any]] = None, query_llm_func: Callable = None) -> List[Dict[str, Any]]:
    ...
    async def get_relevant_memories(self, query: str, user_id: str, all_memories: List[Any]) -> List[Any]:
        ...
        for mem in all_memories:
            ...
            if not mem_id or not mem_content: continue
        ...
        if texts_to_embed:
            ...
            if any(e is not None for e in new_embeddings):
                await self.embedding_manager.store_embeddings_batch_persistent(user_id, ids_to_embed, texts_to_embed, new_embeddings)
    ...
    async def process_memory_operations(self, operations: List[Dict[str, Any]], user_id: str, skip_deduplication: bool = False) -> List[Dict[str, Any]]:
        ...
        for op in operations:
            try:
                ...
                if kind == "NEW" and content:
                    ...
                    try:
                        ...
                        try:
                            if add_memory and (AddMemoryForm or LocalAddMemoryForm):
                                ...
                                if req: mem_obj = await add_memory(request=req, form_data=form, user=user_obj)
                                else: raise ImportError("MockRequest not available")
                            else: raise ImportError("Router add_memory not successfully imported")
                        ...
                        if dedup_embedding is not None and memory_id:
                            ...
                            await self.embedding_manager.store_embedding_persistent(user_id, str(memory_id), content, dedup_embedding)
                    ...
                    updated_memory = Memories.update_memory_by_id_and_user_id(memory_id, user_id, new_content)
                    if updated_memory:
                        ...
                        if new_embedding is not None:
                            ...
                            await self.embedding_manager.store_embedding_persistent(user_id, str(memory_id), new_content, new_embedding)
                            if VECTOR_DB_CLIENT:
                                try:
                                    VECTOR_DB_CLIENT.upsert(collection_name=f"user-memory-{user_id}", items=[{"id": str(memory_id), "text": new_content, "vector": new_embedding.tolist() if hasattr(new_embedding, "tolist") else new_embedding}])
                                except Exception: pass
                        success_ops.append(op)
                    ...
                    if VECTOR_DB_CLIENT:
                        try: VECTOR_DB_CLIENT.delete(collection_name=f"user-memory-{user_id}", ids=[str(memory_id)])
                        except Exception: pass
                    success_ops.append(op)
    ...
    async def _is_duplicate(self, text: str, user_id: str, exclude_id: str = None, all_memories_override: List[Any] = None) -> Tuple[bool, Optional[np.ndarray]]:
        if not text or not self.valves.deduplicate_memories: return False, None
        try:
            if all_memories_override is not None: all_memories = all_memories_override
            else: all_memories = Memories.get_memories_by_user_id(user_id)
            if not all_memories: return False, None

            if self.valves.use_embeddings_for_deduplication:
                new_embedding = await self.embedding_manager.get_embedding(text)
                if new_embedding is None:
                    return await self._check_text_similarity(text, all_memories, exclude_id=exclude_id), None

                for i, memory in enumerate(all_memories):
                    memory_id = memory.id if hasattr(memory, "id") else memory.get("id")
                    if exclude_id and str(memory_id) == str(exclude_id): continue
                    ...
                    if existing_embedding is None:
                        ...
                        if existing_embedding is not None:
                            ...
                            if existing_embedding is not None:
                                await self.embedding_manager.cache.set(memory_id, existing_embedding)
                                await self.embedding_manager.store_embedding_persistent(user_id, memory_id, content_for_embedding, existing_embedding)
    ...
    async def _check_text_similarity(self, text: str, all_memories: List[Any], exclude_id: str = None) -> bool:
        ...
        for i, memory in enumerate(all_memories):
            ...
            if similarity >= self.valves.similarity_threshold: return True
        return False

    async def cluster_and_summarize(self, user_id: str, query_llm_func: Callable) -> Optional[str]:
        try:
            memories = Memories.get_memories_by_user_id(user_id)
            if not memories or len(memories) < self.valves.summarization_min_cluster_size: return
        ...
        for i, (memory_id, content) in enumerate(zip(ids, contents, strict=True)):
            ...
            if cached_embedding is not None: embeddings.append(cached_embedding)
            else:
        ...
        if uncached_contents:
            ...
            await self.embedding_manager.store_embeddings_batch_persistent(user_id, [str(ids[idx]) for idx in uncached_indices], uncached_contents, new_embeddings)

        valid_indices = [i for i, e in enumerate(embeddings) if e is not None]
        if len(valid_indices) < self.valves.summarization_min_cluster_size: return
        ...
        for i in valid_indices:
            ...
            for j in valid_indices:
                ...
                if self._cosine_similarity(embeddings[i], embeddings[j]) >= self.valves.summarization_similarity_threshold:
                    cluster.append(j)
                    visited.add(j)
            if len(cluster) >= self.valves.summarization_min_cluster_size: clusters.append(cluster)
        ...
        for cluster_indices in clusters:
            try:
                ...
                if summary:
                    op = {"operation": "NEW", "content": summary, "tags": ["summary"], "memory_bank": "General", "confidence": 1.0}
                    ...
                    if success_ops:
                        for m in cluster_memories:
                            try:
                                ...
                                if VECTOR_DB_CLIENT: VECTOR_DB_CLIENT.delete(collection_name=f"user-memory-{user_id}", ids=[str(m.id)])
                            except Exception: pass
...
class TaskManager:
    ...
    async def _scavenge_rogue_tasks(self):
        ...
        for task in all_tasks:
            ...
            if any(x in repr(task) for x in ["_summarize_old_memories_loop", "function_adaptive_memory"]):
                if task not in self.tasks: task.cancel()
...
class Filter:
    class Valves(BaseModel):
        ...
        embedding_provider_type: Literal["local", "openai_compatible", "ollama"] = Field(default="ollama", description="Provider for embedding generation")
        embedding_model_name: str = Field(default="nomic-embed-text", description="Embedding model name")
        embedding_api_url: Optional[str] = Field(default="http://host.docker.internal:11434", description="Embedding API URL")
        embedding_api_key: Optional[str] = Field(default=None, description="API Key")

        enable_summarization_task: bool = Field(default=True, description="Enable summarization task")
        summarization_interval: int = Field(default=3600, description="Summarization interval (1h)")
        enable_error_logging_task: bool = Field(default=True, description="Enable error logging")
        error_logging_interval: int = Field(default=1800, description="Error log interval")
        enable_vector_cleanup_task: bool = Field(default=True, description="Enable vector cleanup")
        vector_cleanup_interval: int = Field(default=7200, description="Vector cleanup interval")
        ...
        summarization_memory_prompt: str = "Summarize these user memories into a single concise paragraph."
        ...
        max_total_memories: int = Field(default=1000, description="Max memories per user")
        pruning_strategy: Literal["fifo", "least_relevant"] = "fifo"
        min_memory_length: int = 8
        min_confidence_threshold: float = Field(default=0.65, description="Min confidence to save")
        recent_messages_n: int = Field(default=10, description="Messages context for extraction")
        save_relevance_threshold: float = 0.8
        max_injected_memory_length: int = Field(default=2000, description="Max extracted memory context length")
        ...
        related_memories_n: int = Field(default=8, description="Number of memories to retrieve")
        relevance_threshold: float = Field(default=0.55, description="Min relevance for retrieval")
        ...
        memory_identification_prompt: str = """You are an automated JSON data extraction system. Your ONLY function is to identify user-specific facts... [Truncated for brevity, using original prompt]"""
        memory_relevance_prompt: str = """You are a memory retrieval assistant... [Truncated]"""
        memory_merge_prompt: str = """You are a memory consolidation assistant... [Truncated]"""
        ...
        for attempt in range(valves.max_retries + 1):
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    headers = {"Content-Type": "application/json"}
                    if valves.llm_api_key: headers["Authorization"] = f"Bearer {valves.llm_api_key}"
                    payload = {"model": valves.llm_model_name, "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}], "stream": False}
                ...
                if attempt < valves.max_retries: await asyncio.sleep(valves.retry_delay)
                else: self.error_manager.increment("llm_call_errors")
        return None

    async def inlet(self, body: Dict[str, Any], __event_emitter__=None, __user__=None) -> Dict[str, Any]:
        if not __user__ or not body.get("messages"): return body
        raw_valves = __user__.get("valves", {})
        user_valves = self.UserValves(**raw_valves) if isinstance(raw_valves, dict) else self.UserValves()
        if not user_valves.enabled: return body
        ...
        if isinstance(last_message, list): last_message = " ".join([m.get("text", "") for m in last_message if m.get("type") == "text"]).strip()
        if not last_message or last_message.startswith("/"): return body

        pipeline = MemoryPipeline(self.valves, self.embedding_manager, self.error_manager)
        try: all_memories = Memories.get_memories_by_user_id(user_id)
        except Exception: all_memories = []
        ...
        if relevant_memories:
            ...
            if messages[0]["role"] == "system": messages[0]["content"] += f"\n\n{context_text}"
            else: messages.insert(0, {"role": "system", "content": context_text})

            if user_valves.show_status:
                count = len(relevant_memories)
                if count > 0:
                    status_dict = {"type": "status", "data": {"description": f"🧠 Recalled {count} memories.", "done": True}}
                    if __event_emitter__: await __event_emitter__(status_dict)
        return body
    ...
    async def outlet(self, body: Dict[str, Any], __event_emitter__=None, __user__=None) -> Dict[str, Any]:
        ...
        if user_message:
            ...
            success_ops = []
            if ops: success_ops = await pipeline.process_memory_operations(ops, user_id)
            ...
            if user_valves.show_status and __event_emitter__ and success_ops:
                for op in success_ops:
                    content = op.get("content", "")
                    bank = op.get("memory_bank", "General")
                    await __event_emitter__({"type": "status", "data": {"description": f"📝 {content} ({bank})", "done": True}})
                
                count = len(success_ops)
                final_msg = f"🧠 Saved {count} memories"
                await __event_emitter__({"type": "status", "data": {"description": final_msg, "done": True}})
    ...
    async def cleanup_orphaned_vectors(self, user_id: str):
        if not VECTOR_DB_CLIENT: return
        try:
            ...
            if result and "ids" in result:
                ...
                if orphaned_ids:
                    VECTOR_DB_CLIENT.delete(collection_name=collection_name, ids=orphaned_ids)
